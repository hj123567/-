...
public interface DormRoomService extends IService<DormRoom> {
    ...
    // 查询指定房间的空床位
    List<Map<String, Object>> getEmptyBeds(Integer dormRoomId);

    // 查询所有可换宿舍的空床位
    List<Map<String, Object>> getAllEmptyBeds(Integer dormBuildId);

    // 查询所有可换宿舍的空床位（排除当前房间）
    List<Map<String, Object>> getAllEmptyBedsExcludeCurrent(Integer dormBuildId, Integer currentRoomId);

    // 同步床位数据：根据学生表的宿舍信息重新分配房间表的床位
    int syncBedData();

    // 智能重新分配宿舍：确保每个宿舍最多4人，男女分开
    String smartAssignDorm();

    // 获取所有有空床位的房间列表（用于新生分配）
    List<Map<String, Object>> getRoomsWithEmptyBeds(Integer dormBuildId);

    // 为学生分配床位
    int assignBedToStudent(String username, Integer dormBuildId, Integer dormRoomId, Integer bedNum);
}
