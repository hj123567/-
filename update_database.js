...
connection.connect((err) => {
    ...
    connection.query(addColumnsSql, (err, results) => {
        if (err) {
        ...
        studentData.forEach((data, index) => {
            ...
            connection.query(updateSql, [data[1], data[2], data[3], data[4], data[0]], (err, result) => {
                ...
                if (completed === total) {
                    ...
                    console.log('\nStudent entity updated successfully!');
                    console.log('Please restart the backend service now.');
                }
            });
        });
    });
